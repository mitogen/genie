...
void LocalAssemblyReadEncoder::addSingleRead(const format::sam::SamRecord &rec,
...
format::mpegg_rec::MpeggRecord::ClassType LocalAssemblyReadEncoder::addRead(const format::sam::SamRecord &rec,
...
format::mpegg_rec::MpeggRecord::ClassType LocalAssemblyReadEncoder::addPair(const format::sam::SamRecord &rec1,
                                                                            const std::string &ref1,
                                                                            const format::sam::SamRecord &rec2,
...
void LocalAssemblyReadEncoder::codeVariants(const std::string &read, const std::string &cigar, const std::string &ref,
                                            format::mpegg_rec::MpeggRecord::ClassType type, bool isFirst) {
    size_t count = 0;
    size_t read_pos = 0;
    size_t ref_offset = 0;
        ...
        switch (cigar[cigar_pos]) {
            ...
            case 'I':
                for (size_t i = 0; i < count; ++i) {
                    ...
                    container->mmtype_2.push_back(
                        format::mpegg_p2::getAlphabetProperties(format::mpegg_p2::ParameterSet::AlphabetID::ACGTN)
                            .inverseLut[read[read_pos]]);
                    read_pos++;
                }
                break;
...
format::mpegg_rec::MpeggRecord::ClassType LocalAssemblyReadEncoder::getClass(const std::string &read,
