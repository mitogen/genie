...
void pack(const ProgramOptions& programOptions, std::unique_ptr<StreamContainer> container, uint32_t read_length,
    ...
    for (size_t descriptor = 0; descriptor < format::mpegg_p2::NUM_DESCRIPTORS; ++descriptor) {
        for (size_t subdescriptor = 0;
             ...
             ++subdescriptor) {
            compress(configs[descriptor][subdescriptor], &raw_data[descriptor][subdescriptor],
    ...
    ParameterSet ps =
        createQuickParameterSet(PARAMETER_SET_ID, READ_LENGTH, paired, QV_PRESENT, programOptions.type, configs, false);
    ...
    AccessUnit au = createQuickAccessUnit(ACCESS_UNIT_ID, PARAMETER_SET_ID, readNum, programOptions.type,
                                          DataUnit::DatasetType::ALIGNED, &generated_streams);
    au.write(&bw);
}
