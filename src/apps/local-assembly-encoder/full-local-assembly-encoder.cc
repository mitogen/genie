...
namespace lae {
FullLocalAssemblyEncoder::FullLocalAssemblyEncoder(uint32_t cr_buf_max_size, bool _debug)
    : refCoder(cr_buf_max_size), readCoder(), debug(_debug) {}
...
std::unique_ptr<StreamContainer> FullLocalAssemblyEncoder::pollStreams() { return readCoder.pollStreams(); }
}  // namespace lae