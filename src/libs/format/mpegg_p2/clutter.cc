...
ParameterSet createQuickParameterSet(uint8_t _parameter_set_id, uint8_t _read_length, bool paired_end,
    ...
    ParameterSet ret(_parameter_set_id, _parameter_set_id, dataType, ParameterSet::AlphabetID::ACGTN, _read_length,
                     paired_end, false, qv_values_present, 0, false, false);
    ret.addClass(type, util::make_unique<qv_coding1::QvCodingConfig1>(qv_coding1::QvCodingConfig1::QvpsPresetId::ASCII,
                                                                      reverse_flag));
...
void write8bit(uint8_t val, std::vector<uint8_t> *ret) { ret->push_back(val); }
...
void writeu7v(uint32_t val, std::vector<uint8_t> *ret) {
    ...
    for (int i = tmp.size() - 1; i > 0; i--) ret->push_back(tmp[i] | 0x80);
    ret->push_back(tmp[0]);
}

void insert(gabac::DataBlock *transformed_subsequence, std::vector<uint8_t> *ret) {
    ret->insert(
        ret->end(), reinterpret_cast<const uint8_t *>(transformed_subsequence->getData()),
...
std::vector<uint8_t> create_payload_tokentype(std::vector<std::vector<gabac::DataBlock>> *block,
                                              uint32_t records_count) {
    std::vector<uint8_t> ret;
    if (block->size() != 128 * 6) throw std::runtime_error("tokentype: number of tokens not 128*6 as expected.");
    ...
    for (uint16_t i = 0; i < 128 * 6; i++) {
        if ((*block)[i].size() != 0) num_tokentype_sequences++;
    }
...
AccessUnit createQuickAccessUnit(uint32_t access_unit_id, uint8_t parameter_set_id, uint32_t reads_count,
                                 ...
                                 uint32_t records_count) {
    AccessUnit au(access_unit_id, parameter_set_id, auType, reads_count, datatype, 32, 32, 0);
