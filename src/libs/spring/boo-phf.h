        ...
        while (partial >= steps) {
            if (timer_mode) {
                ...
                fprintf(stderr,
                        ...
                        "%3i min %-2.0f sec",
                        13, message.c_str(), 100 * (double)done / todo, min_e, elapsed, min_r, rem);
        ...
        while (partial_threaded[tid] >= steps) {
            if (timer_mode) {
                ...
                fprintf(stderr,
                        "%c[%s]  %-5.3g%%   elapsed: %3i min %-2.0f sec   remaining: "
                        "%3i min %-2.0f sec",
                        13, message.c_str(), 100 * (double)total_done / todo, min_e, elapsed, min_r, rem);
            } else {
                fprintf(stderr, "-");
                fflush(stderr);
            }
...
class HashFunctors {
   ...
   private:
    ...
    void generate_hash_seed() {
        static const uint64_t rbase[MAXNBFUNC] = {0xAAAAAAAA55555555ULL, 0x33333333CCCCCCCCULL, 0x6666666699999999ULL,
...
class mphf {
   ...
   public:
    ...
    mphf(size_t n, Range const &input_range, int num_thread = 1, double gamma = 2.0, bool writeEach = true,
    ...
    uint64_t totalBitSize() {
        ...
        uint64_t totalsize = totalsizeBitset + _final_hash.size() * 42 * 8;  // unordered map takes approx 42B per elem
        ...
        printf("final hash  %12lu  bits (%.2f %%) (nb in final hash %lu)\n", _final_hash.size() * 42 * 8,
               100 * (float)(_final_hash.size() * 42 * 8) / totalsize, _final_hash.size());
        return totalsize;
    }
...
#ifdef BOOPHF_USE_PTHREADS
            for (int ii = 0; ii < _num_thread; ii++)
                pthread_create(&tab_threads[ii], NULL, thread_processLevel<elem_t, Hasher_t, Range, disklevel_it_type>,
                               &t_arg);  //&t_arg[ii]
